...
'''
Modified Monomial and Polynomial classes from
https://github.com/keon/algorithms/blob/master/algorithms/maths/polynomial.py , 
in order to perform symbolic math.
'''
...
class Monomial:
    ...
    def equal_upto_scalar(self, other):
        """
...
CIPS = {0: Polynomial([Monomial({},1)]), 1: Polynomial([Monomial({1:1},1)])}
def cycle_index_polynomial_sn(n):
    '''
    Get the cycle index polynomial of S_n.
    Using recurrence relation from:
    https://wiki.algo.is/Cycle%20index#:~:text=The%20cycle%20index%20of%20the%20symmetric%20group%20Sn%20in,kjkjk!
    '''
    if n in CIPS:
        return CIPS[n] 
    else:
        ...
        for l in range(1, n+1):
            s += Polynomial([Monomial({l: 1}, comb(n-1,l-1) * (f(l)//l) * (f(n-l)))]) * cycle_index_polynomial_sn(n-l)
        
        CIPS[n] = s.__truediv__(Monomial({},f(n)))
        return CIPS[n]


def cycle_index_polynomial_sn_x_sm(n,m,s):
    '''
    Using https://www.sciencedirect.com/science/article/pii/0012365X9390015L (2.7), and
    ...
    I found the cycle index polynomial of S_w x S_h using the cycle index polynomial of S_w and S_h.
    '''
    if n == 1:
        return cycle_index_polynomial_sn(m).subs(s)
    elif m == 1:
        return cycle_index_polynomial_sn(n).subs(s)

    p1 = cycle_index_polynomial_sn(n)
    p2 = cycle_index_polynomial_sn(m)

    suma = 0
    for mon1 in p1.monomials:
        for mon2 in p2.monomials:
            p_vars = dict()
            var1 = mon1.variables
            var2 = mon2.variables
            ...
            for v1 in var1:
                for v2 in var2:
                    ...
                    if lcm(v1, v2) not in p_vars:
                        p_vars[lcm(v1, v2)] = 0
                    
                    p_vars[lcm(v1, v2)] += var1[v1] * var2[v2] * (v1*v2//lcm(v1, v2))

            p = Monomial(p_vars, mon1.coeff * mon2.coeff).substitute(s)
            suma += p
...
def solution(w, h, s):
    '''
    Using Burnside's lemma, I found that this problem is equivalent to evaluating the 
    cycle index polynomial of S_w x S_h at x_i=s. Where S is the symmetric group.
    '''
    return str(cycle_index_polynomial_sn_x_sm(w, h, s))